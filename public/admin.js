...
function renderCms(){const s=state.settings||{},b=s.branding||{},a=s.appearance||{},st=s.stock||{},q=s.quotes||{},sp=s.supplierPortal||{},nf=s.nfe||{},m=s.modules||{};
  setVal('cfgSystemName',b.systemName);setVal('cfgModuleName',b.moduleName);setVal('cfgShortName',b.shortName);if(String(b.logoUrl||'').startsWith('data:image/')){setVal('cfgLogoData',b.logoUrl);setVal('cfgLogoUrl','')}else{setVal('cfgLogoData','');setVal('cfgLogoUrl',b.logoUrl)};renderLogoPreview();setVal('cfgCompanyName',b.companyName);setVal('cfgDocumentName',b.documentName);setVal('cfgCnpj',b.cnpj);setVal('cfgPhone',b.phone);setVal('cfgEmail',b.email);setVal('cfgAddress',b.address);
...
async function optimizeLogoFile(file){
  ...
  const src=await new Promise((resolve,reject)=>{const r=new FileReader();r.onload=()=>resolve(r.result);r.onerror=()=>reject(new Error('Não foi possível ler a imagem.'));r.readAsDataURL(file)});
  const img=await new Promise((resolve,reject)=>{const i=new Image();i.onload=()=>resolve(i);i.onerror=()=>reject(new Error('Imagem inválida.'));i.src=src});
...
async function saveSettings(){
  ...
  try{
    ...
    let result;
    try{result=await api('/api/settings',{method:'PUT',body:payload})}
    catch(firstError){
      // PUT é idempotente: uma segunda tentativa é segura e ajuda em cold starts do serverless/Neon.
      await new Promise(r=>setTimeout(r,700));
      result=await api('/api/settings',{method:'PUT',body:payload});
    }
    state.settings=result.settings;
    applySettings();renderAll();toast('Configurações salvas no Neon');
  }catch(e){
    openModal('Erro ao salvar',`<p>${esc(e.message)}</p><p class="muted">A conexão com o servidor não concluiu a gravação. Tente novamente; se persistir, abra os logs da função na Vercel.</p>`)
